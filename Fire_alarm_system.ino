...
#define GREEN_LED_PIN 12  // GPIO pin for green LED (default ON, second one for alarm)
#define BUZZER_PIN 13     // GPIO pin for buzzer
#define FIRE_SENSOR_PIN 35 // Analog pin for fire sensor (MH-Sensor-Series)
#define SMOKE_SENSOR_PIN 34 // Analog pin for smoke sensor (MQ-2)
...
// Fire and Smoke sensor thresholds (adjust based on sensor testing)
int fireThreshold = 2500;  // Fire sensor threshold (adjust as per your test)
int smokeThreshold = 500;  // Smoke sensor threshold (adjust as per your test)
...
void setup() {
  ...
  // Start with the first Green LED ON (default state)
...
void loop() {
  ...
  if (fireAverage > fireThreshold || smokeAverage > smokeThreshold) {
    ...
    if (!alarmActive) {
      ...
      digitalWrite(GREEN_LED_PIN, LOW);    // Green LED OFF (first one)
  ...
  // Check if the alarm duration has passed (10 seconds)
  if (alarmActive && (millis() - alarmStartTime >= 10000)) {
    alarmActive = false;  // Reset alarm state
    digitalWrite(RED_LED_PIN, LOW);      // Red LED OFF
    digitalWrite(BUZZER_PIN, LOW);       // Buzzer OFF
    digitalWrite(GREEN_LED_PIN, HIGH);   // Green LED ON (second one, for alarm)
